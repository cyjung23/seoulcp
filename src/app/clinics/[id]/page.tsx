...
async function getData(id: number) {
  ...
  const { data: deviceRows } = await supabase
    .from("clinic_devices")
    .select("device_id, devices(id, device_name_ko, device_name_en, category_ko)")
    .eq("clinic_id", id);

  const { data: treatmentRows } = await supabase
    .from("clinic_treatments")
    .select("treatment_id, price_krw, description, treatments(id, name_ko, name_en, category_ko)")
    .eq("clinic_id", id);
  ...
  return {
    ...
    treatments: (treatmentRows || []).map((r: any) => ({
      ...r.treatments,
...
export default async function ClinicDetailPage({
  ...
  const deviceCategories = [...new Set(devices.map((d: any) => d.category_ko || "기타"))].sort();
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="max-w-5xl mx-auto py-8 px-6">
        ...
        <div className="border rounded-xl p-6 mb-8">
          ...
          {clinic.address_ko && <p className="text-gray-600 mb-2">📍 {clinic.address_ko}</p>}
          {clinic.phone && <p className="text-gray-600 mb-2">📞 {clinic.phone}</p>}
          {clinic.operating_hours && <p className="text-gray-600 mb-2">🕐 {clinic.operating_hours}</p>}
          {clinic.website && (
        ...
        {devices.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xl font-bold mb-4">🔬 보유 장비 ({devices.length})</h2>
        ...
        {treatments.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xl font-bold mb-4">💉 시술 목록 ({treatments.length})</h2>
            {treatmentCategories.map((cat) => (
              <div key={cat} className="mb-4">
                <h3 className="font-semibold text-gray-700 mb-2">{cat}</h3>
                <div className="flex flex-wrap gap-2">
                  {treatments
                    .filter((t: any) => (t.category_ko || "기타") === cat)
                    .map((t: any) => (
                      <Link
                        key={t.id}
        ...
        {doctors.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xl font-bold mb-4">👨‍⚕️ 의료진 ({doctors.length})</h2>
                ...
                <div key={doc.id} className="border rounded-lg p-4">
                  ...
                  {doc.doctor_specialties?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {doc.doctor_specialties.map((s: any, idx: number) => (
